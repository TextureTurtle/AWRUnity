...
{
	public class FractalNoiseSingle
	{
		private PerlinSingle m_Noise;
		...
		public FractalNoiseSingle(float inH, float inLacunarity, float inOctaves)
			: this(inH, inLacunarity, inOctaves, null)
		{
		}

		public FractalNoiseSingle(float inH, float inLacunarity, float inOctaves, PerlinSingle noise)
		{
			...
			{
				m_Exponent[i] = Mathf.Pow(m_Lacunarity, 0f - inH);
				num *= m_Lacunarity;
			}
			if (noise == null)
			{
				m_Noise = new PerlinSingle();
			}
			else
			{
				m_Noise = noise;
