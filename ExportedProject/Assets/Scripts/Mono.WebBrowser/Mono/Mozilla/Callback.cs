...
namespace Mono.Mozilla
{
	internal class Callback
	{
		...
		public void OnStateChange(nsIWebProgress progress, nsIRequest request, int status, uint state)
		{
			...
			{
				nsIDOMWindow ret;
				progress.getDOMWindow(out ret);
				nsIChannel nsIChannel2 = (nsIChannel)request;
				nsIURI ret2;
				nsIChannel2.getURI(out ret2);
				if (ret2 == null)
				{
			...
			{
				nsIDOMWindow ret3;
				progress.getDOMWindow(out ret3);
				nsIChannel nsIChannel3 = (nsIChannel)request;
				nsIURI ret4;
				nsIChannel3.getURI(out ret4);
				if (ret4 == null)
				{
					currentUri = "about:blank";
				}
			...
			{
				nsIDOMWindow ret7;
				progress.getDOMWindow(out ret7);
				nsIChannel nsIChannel4 = (nsIChannel)request;
				nsIURI ret8;
				nsIChannel4.getURI(out ret8);
				if (ret8 == null)
				{
					currentUri = "about:blank";
					return;
			...
			else if (flag5 && !flag6 && !flag7 && flag8 && flag9)
			{
				...
				{
					nsIDOMWindow ret9;
					progress.getDOMWindow(out ret9);
			...
			else
			{
				...
				nsIDOMWindow ret10;
				progress.getDOMWindow(out ret10);
				nsIDOMDocument ret11;
				ret10.getDocument(out ret11);
				if (ret11 != null)
				{
					...
					if (owner.documents.ContainsKey(hashCode))
					{
						...
						EventHandler eventHandler = (EventHandler)document.Events[Document.LoadStopped];
		...
		public void OnShowContextMenu(uint contextFlags, [MarshalAs(UnmanagedType.Interface)] nsIDOMEvent eve, [MarshalAs(UnmanagedType.Interface)] nsIDOMNode node)
		{
			...
			if (contextMenuEventHandler != null)
			{
				...
				int ret;
				nsIDOMMouseEvent2.getClientX(out ret);
				int ret2;
				nsIDOMMouseEvent2.getClientY(out ret2);
